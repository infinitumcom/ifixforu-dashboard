"""
LV 门店 Dashboard Bot — Telegram 长轮询
接收群消息 → Claude 分类 → 存入 SQLite → 看板显示

启动: python3 dashboard_bot_lv.py
"""
...
LV_BOT_TOKEN = "8590394374:AAFrBZmN05Zfhcai9etxRqsF4mt9UiywTSM"
LV_STORE_CODE = "las_vegas"
TELEGRAM_API = f"https://api.telegram.org/bot{LV_BOT_TOKEN}"

# PID 锁文件，防止重复实例
PID_FILE = Path(__file__).parent / "dashboard_bot_lv.pid"
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(os.path.dirname(os.path.abspath(__file__)), "dashboard_bot_lv.log")),
    ],
)
...
async def cmd_list(chat_id: str) -> None:
    """显示今日看板摘要"""
    items = get_today_items(LV_STORE_CODE)
    if not items:
...
async def cmd_notices(chat_id: str) -> None:
    """显示当前公告"""
    notices = get_active_notices(LV_STORE_CODE)
    if not notices:
...
async def cmd_wechat(chat_id: str, args: str, msg_id: int, sender_name: str) -> None:
    ...
    add_item(LV_STORE_CODE, item)
    wechat = get_wechat_total(LV_STORE_CODE)
    await send_message(
        chat_id,
...
async def _process_one_item(chat_id: str, msg_id: int, result: dict, sender_name: str) -> None:
    ...
    if msg_type == "notice":
        notice_id = add_notice(LV_STORE_CODE, result)
        await send_message(
            chat_id,
        ...
        if source_mode == "total":
            current = get_wechat_total(LV_STORE_CODE)
        ...
        item_id = add_item(LV_STORE_CODE, result)
        wechat = get_wechat_total(LV_STORE_CODE)
        await send_message(
            chat_id,
            f"💚 微信好友已记录: +{amount}人 (今日+{wechat['today_added']}，累计{wechat['total']:,})",
...
async def poll_updates() -> None:
    ...
    init_db()
    acquire_pid_lock()
    logger.info("LV Dashboard Bot 已启动 (store: %s, PID: %d)", LV_STORE_CODE, os.getpid())
